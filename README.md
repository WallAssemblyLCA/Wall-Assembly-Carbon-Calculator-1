...
`requirements.txt` pins Streamlit 1.60 and keeps Starlette below 1.4. Starlette 1.4 changed its GZip responder interface, which causes the `thread_minimum_size` startup error with Streamlit 1.60. Keep this pin until a later Streamlit version explicitly supports that Starlette change.
