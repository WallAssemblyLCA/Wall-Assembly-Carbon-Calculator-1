...
- The Studio G Architects logo is omitted from the Streamlit web interface and remains applied to exported graphics, Excel worksheets, and PDF report pages.
...
- Copyright and ownership notices remain in the interface footer and exported files; the separate Legal Notice navigation page has been removed.
...
Keep the `assets` folder beside `app.py` when deploying so the logo can be used in charts and exported Excel/PDF files.
...
## Branding display fix

- Corrected the page hero and legal footer HTML so Streamlit renders the branded layout instead of displaying the HTML tags as code.
- Embedded the Studio G Architects logo inside `app.py` as a fallback. The logo still uses `assets/studio_g_logo.jpg` when available, but branding will now continue to display even if the assets folder is accidentally omitted during deployment.

## Interface cleanup

- Removed the Studio G logo from the main page headers and sidebar.
- Removed the Legal Notice navigation tab and page.
- Kept the copyright footer and ownership language.
