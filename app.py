# Copyright (c) 2026 Studio G Architects. All rights reserved.
#
...
LOGO_PATH = APP_DIR / "assets" / "studio_g_logo.jpg"
COPYRIGHT_YEAR = datetime.now().year
COPYRIGHT_SHORT = f"© {COPYRIGHT_YEAR} Studio G Architects. All rights reserved."
PROPRIETARY_NOTICE = (
...
def render_page_hero(title: str, subtitle: str | None = None) -> None:
    """Render a consistently branded page hero."""
    logo_html = (
        f'<img class="hero-logo" src="{LOGO_DATA_URI}" alt="Studio G Architects logo">'
    ...
    st.markdown(
        f'''<div class="hero">
          ...
          {subtitle_html}
          <div class="hero-copyright">{html.escape(COPYRIGHT_SHORT)}</div>
        </div>''',
        unsafe_allow_html=True,
    )
...
def render_legal_footer() -> None:
    """Render the ownership notice at the bottom of every app page."""
    logo_html = (
        f'<img src="{LOGO_DATA_URI}" alt="Studio G Architects logo">'
        if LOGO_DATA_URI else ""
    )
    st.markdown(
        f'''<div class="legal-footer">
          {logo_html}
          <div><strong>{html.escape(COPYRIGHT_SHORT)}</strong><br>
          {html.escape(PROPRIETARY_NOTICE)}<br>
          {html.escape(THIRD_PARTY_NOTICE)} {html.escape(RESULTS_DISCLAIMER)}</div>
        </div>''',
        unsafe_allow_html=True,
    )
...
def brand_plotly_figure(fig) -> None:
    ...
    fig.add_annotation(
        ...
        x=1.0,
        y=-0.145,
        text=html.escape(COPYRIGHT_SHORT),
        showarrow=False,
        xanchor="right",
        yanchor="top",
    ...
    fig.update_layout(margin=dict(b=max(72, int(existing_bottom))))


def branded_csv_bytes(dataframe: pd.DataFrame) -> bytes:
    """Export tabular results with an explicit Studio G ownership notice."""
    export = dataframe.copy()
    export.insert(0, "Copyright", COPYRIGHT_SHORT)
    export.insert(1, "Use Notice", PROPRIETARY_NOTICE)
    export["Third-Party Rights"] = THIRD_PARTY_NOTICE
...
def add_excel_branding(ws, title: str) -> None:
    """Add the Studio G logo, ownership notice, and print footer to a worksheet."""
    ...
    copyright_cell = ws.cell(2, 4, COPYRIGHT_SHORT)
    copyright_cell.font = Font(size=10, bold=True, color="002F65")
    copyright_cell.alignment = Alignment(vertical="center", wrap_text=True)

    ws.merge_cells(start_row=3, start_column=4, end_row=3, end_column=10)
    notice_cell = ws.cell(3, 4, PROPRIETARY_NOTICE + " " + THIRD_PARTY_NOTICE)
    notice_cell.font = Font(size=8, italic=True, color="64748B")
    notice_cell.alignment = Alignment(vertical="top", wrap_text=True)
...
def comparison_report_bytes() -> bytes:
    ...
    story.extend([
        ...
        Paragraph(pdf_safe_text(COPYRIGHT_SHORT), legal_style),
    ...
    def add_footer(canvas, document) -> None:
        ...
        canvas.drawCentredString(page_width / 2, 8, "Proprietary Studio G Architects report - third-party data remains subject to source rights.")
        canvas.restoreState()
...
with st.sidebar:
    if LOGO_DATA_URI:
        st.markdown(
            f'<div class="sidebar-logo-wrap"><img src="{LOGO_DATA_URI}" alt="Studio G Architects logo" style="width:100%;height:auto;display:block;"></div>',
            unsafe_allow_html=True,
        )
    ...
    page_options = ["Build Assemblies", "Compare Assemblies", "Material Library", "Definitions", "Method Guide", "Legal Notice"]
    page = st.radio(
        "Navigation",
        page_options,
    ...
    st.markdown(
        f'<div class="sidebar-copyright">{html.escape(COPYRIGHT_SHORT)}<br>Studio G Architects proprietary application.</div>',
        unsafe_allow_html=True,
    )
...
if page == "Build Assemblies":
    ...
    with saved_col:
        ...
        with st.popover("Saved walls", help="Save and reuse complete wall assemblies"):
            st.caption("Saved walls stay available in this browser.")
            if st.button(
                "Save current wall",
...
# =============================================================================
    ...
    render_page_hero(
        ...
        "Copyright, use restrictions, third-party rights, and results limitations for Wall-E.",
    )

    with st.container(border=True):
        st.markdown(f"### {COPYRIGHT_SHORT}")
        st.write(PROPRIETARY_NOTICE)

    with st.container(border=True):
        st.markdown("### Third-party materials")
        st.write(THIRD_PARTY_NOTICE)

    with st.container(border=True):
        st.markdown("### Results and professional review")
        st.write(RESULTS_DISCLAIMER)

    with st.container(border=True):
        st.markdown("### Permission")
        st.write(
            "Written permission from Studio G Architects is required before reproducing, modifying, "
            "distributing, publishing, sublicensing, or commercially using the proprietary portions "
            "of Wall-E, its reports, graphics, or branded result formats, except where applicable law permits otherwise."
        )
