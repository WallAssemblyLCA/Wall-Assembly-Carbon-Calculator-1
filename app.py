...
except ImportError:  # The app still opens if dependencies have not been refreshed yet.
    sort_items = None
...
def reorder_assemblies(ordered_ids: list[str]) -> bool:
    """Apply a validated wall-option order and report whether it changed."""
    current = st.session_state.assemblies
    current_ids = [assembly["id"] for assembly in current]
    if ordered_ids == current_ids:
        return False
    if len(ordered_ids) != len(current_ids) or set(ordered_ids) != set(current_ids):
        return False

    by_id = {assembly["id"]: assembly for assembly in current}
    st.session_state.assemblies = [by_id[assembly_id] for assembly_id in ordered_ids]
    return True


def render_assembly_reorder() -> None:
    """Render the drag-and-drop wall-option list and persist its order."""
    if len(st.session_state.assemblies) < 2:
        return

    with st.expander("↕ Reorder wall options", expanded=False):
        st.caption(
            "Drag each wall option into the desired position. The new order is used in "
            "the wall selector, comparison charts, Excel export, and PDF report."
        )

        if sort_items is None:
            st.warning(
                "Drag-and-drop requires streamlit-sortables. Install the updated "
                "requirements.txt and restart the app."
            )
            return

        # Numbered labels are unique even when two assemblies have the same name.
        labels = [
            f"{index + 1}. {assembly['name']}"
            for index, assembly in enumerate(st.session_state.assemblies)
        ]
        label_to_id = {
            label: assembly["id"]
            for label, assembly in zip(labels, st.session_state.assemblies)
        }
        ...
        .sortable-item, .sortable-item:hover {{
            ...
            font-size: 15px;
            font-weight: 700;
            box-shadow: 0 2px 8px rgba(0, 47, 101, 0.06);
            cursor: grab;
        }}
        ...
        # Changing the option IDs/order remounts the component with fresh labels.
        component_key = "assembly_order_" + "_".join(
            assembly["id"][:8] for assembly in st.session_state.assemblies
        )
        ...
        if sorted_labels and sorted_labels != labels:
            ordered_ids = [label_to_id[label] for label in sorted_labels if label in label_to_id]
            if reorder_assemblies(ordered_ids):
                st.rerun()
